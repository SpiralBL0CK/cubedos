...
package Sample_Module.API is
   ...
   -- passing system itself (those are defined in Generic_Message_Manager). The status codes
   ...
   -- message priority, and whatever other parameters are appropriate for the message (shown as
   -- a comment below). Replies sent from the module take a receiver address as a (domain ID,
   -- module ID) pair, the request ID that was provided by the sender (or zero), a message
   -- priority, and whatever other parameters are appropriate for the message (not shown below).
   -- Clients of a module do not need to be concerned with exactly how messages to the module or
   -- from the module are encoded.
   --
   -- Note that these functions do *not* send the messages, they only construct them. The caller
   -- must explicity send using the Route_Message procedure in the message manager. This design
   -- reduces coupling and also gives the caller flexibilty to pre-construct messages and send
   -- them later or de-construct received messages lazily.
   --
   ...
   -- reserved as a placeholder and can be used when this feature is not useful. CubedOS ignores
   -- the request IDs. They are entirely a matter between the sending and receiving modules, and
   -- their use is at the discretion of the module developer(s).
   --
   -- Message priorities are currently not used in any way. They are included only as a place
   -- holder for future work. They are always given the default value of System.Default_Priority
   -- for now.
   --
   ...
   procedure A_Reply_Decode
     (Message : in  Message_Record;
      Status  : out Status_Type;                -- Status of the request (from server).
      -- This is where appropriate OUT parameters are defined for the decoded items.
      Decode_Status : out Message_Status_Type)  -- Status of the decoding (from CubedOS).
   with
     Global => null,
